...
## Cel dokumentu
Ustalić sposób komunikowania statusu API podczas wdrożeń lub incydentów: co, kiedy i do kogo raportować (latencja, błędy, dostępność, kontrakty).

## Zakres i granice
Obejmuje: odbiorców (klienci API, partnerzy, zespoły wew.), kanały (status page, mailing, RSS/webhooks), częstotliwość, szablony komunikatów (planned/ongoing/resolved), dane o wpływie i obejściach. Nie obejmuje: technicznych kroków naprawczych (runbooki).

## Wejścia i wyjścia
- Wejścia: monitoring API (SLO), testy kontraktowe, deployment/incident timeline, lista konsumentów, polityki komunikacji.
- Wyjścia: harmonogram update’ów, szablony komunikatów, log statusów, lista ownerów kanałów.
...
- Odbiorcy i kanały (status page, email, webhooki, support)
- Cadence i triggery (start zdarzenia, kamienie, resolved)
- Format komunikatu (endpointy dotknięte, metryki, obejścia, ETA)
- Komunikacja zewnętrzna vs wewnętrzna
- Log statusów i odpowiedzialni
