...
## Cel dokumentu
Zaplanować i wykonać testy bezpieczeństwa API (SAST/DAST/API fuzzing/authz tests), aby wykryć podatności i potwierdzić zgodność ze standardami/org policy.

## Zakres i granice
- Obejmuje: SAST (kodu/konfiguracji), DAST/API fuzzing, testy autoryzacji, brute-force/abuse protection, testy rate limiting, testy błędów/wycieków, walidację logów bez PII.
- Nie obejmuje: pełnego pentestu infrastruktury ani testów UI.

## Wejścia i wyjścia
- Wejścia: specyfikacja API, modele auth (OAuth/OIDC/API keys/mTLS), środowisko testowe, narzędzia (SAST, DAST, fuzzers), profile ruchu, wymagania compliance (PII/PCI), polityka rate limiting.
- Wyjścia: raport z testów (podatności i ryzyka), dowody (zapytania, logi), rekomendacje i plan napraw, status zgodności z OWASP API Top 10/org policy.
...
1. Zakres testów (usługi, endpointy, modele auth, dane testowe)
2. Metody i narzędzia (SAST/DAST/API fuzzing, authz tests, headers/JWT)
3. Scenariusze testowe: OWASP API Top 10, authz bypass, input validation, rate limiting, error handling, logging/leaks
4. Wyniki i klasyfikacja ryzyk (CVSS/OWASP), dowody
5. Rekomendacje i plan napraw (owner, SLA, retest)
6. Weryfikacja po naprawie (retest, regression, log review)

## Wymagane rozwinięcia
- Lista endpointów + wymagane testy authz (rola/claim/scope), negatywne case.
- Plan fuzzingu i testów rate limiting/bot/abuse.
- Kryteria zaliczenia (brak High/Med, retest dla istotnych zmian).
- Polityka danych testowych (brak real PII, maskowanie, cleanup).

## Wymagane streszczenia
- Streszczenie ryzyk High/Med z terminami napraw i odpowiedzialnymi.
- Streszczenie zgodności z OWASP API Top 10 i wymaganiami org.
