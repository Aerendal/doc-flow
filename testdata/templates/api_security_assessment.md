...
## Cel dokumentu
Ocenić bezpieczeństwo API (architektura, implementacja, operacje), zidentyfikować ryzyka i zalecić remediację przed/po go-live.

## Zakres i granice
- Obejmuje: authn/authz, input validation, rate limiting, error handling, logging, secrets/keys, transport security, dependency risk, testy bezpieczeństwa.
- Nie obejmuje: pełnej oceny infrastruktury sieciowej ani bezpieczeństwa aplikacji poza API (oddzielne audyty).

## Wejścia i wyjścia
- Wejścia: specyfikacja API (OpenAPI/GraphQL), architektura, modele auth (OAuth/OIDC/API keys/mTLS), konfiguracje gateway/WAF, lista zależności, środowiska testowe, wymagania compliance (PCI/PII), logi/trace z testów.
- Wyjścia: raport z oceną ryzyk, lista podatności i priorytetów, rekomendacje remediacji, wynik testów (DAST/SAST), status zgodności z org standardem.
...
1. Zakres i model API (spec, authn/authz, środowiska)
2. Wyniki oceny (podatności, klasyfikacja ryzyk, zgodność ze standardami)
3. Testy bezpieczeństwa (SAST/DAST/API fuzzing, JWT/headers, mTLS)
4. Misconfiguration/OWASP API Top 10 coverage
5. Logowanie/monitoring/alerting bezpieczeństwa
6. Rekomendacje i plan remediacji (właściciele, SLA, weryfikacja)

## Wymagane rozwinięcia
- Lista endpointów i modeli auth (OAuth flows, scopes, API keys/mTLS).
- Pokrycie OWASP API Top 10 (każdy punkt: czy dotyczy? test? wynik?).
- Polityka rate limiting/throttling per klient/tenant.
- Polityka logowania bezpieczeństwa (co logować, bez PII/sekretów).
- Plan remediacji z priorytetami (High/Med/Low) i SLA napraw.

## Wymagane streszczenia
- Streszczenie ryzyk High/Medium z terminami i właścicielami.
- Streszczenie zgodności z OWASP API Top 10 i wymaganiami compliance (PCI/PII).
