...
## Cel dokumentu
Przeanalizować incydent dotyczący API (dostępność, latencja, błędy, zgodność kontraktów), zidentyfikować przyczyny i zaplanować trwałe usprawnienia.

## Zakres i granice
Obejmuje: timeline, wpływ na klientów/partnerów, root cause (kod/infra/kontrakt/rate limit/cache), regresje kompatybilności, CAPA, zmiany w monitoringu/testach kontraktowych. Nie obejmuje: ogólnej strategii API (API Design Spec).

## Wejścia i wyjścia
- Wejścia: logi API (latencja, 4xx/5xx), trace’y, testy kontraktowe, deployment timeline, change list, alerty SLO.
- Wyjścia: postmortem API, lista CAPA (kod, rate limits, cache, retry/idempotencja), aktualizacje testów/monitoringu, decyzje dot. wersjonowania/rollback.
...
- Streszczenie i wpływ (SLO, partnerzy, endpointy)
- Timeline (deploy, ruch, błędy, decyzje)
- Root cause + contributing (kontrakt, schema, throttling, cache, upstream)
- Testy/monitoring: co zadziałało/co zawiodło (contract, canary, shadow)
- CAPA: kod, konfiguracja, limity, retry/circuit breaker, testy kontraktowe
- Zmiany w API versioning/compat, komunikacja do konsumentów
