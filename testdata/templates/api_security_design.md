...
## Cel dokumentu
Zaplanować bezpieczeństwo API w architekturze: model authn/authz, separacja tenantów, ochrony przed nadużyciami, logowanie/monitoring, zgodność z standardami.

## Zakres i granice
- Obejmuje: wybór modeli auth (OAuth/OIDC/API keys/mTLS), autoryzację (RBAC/ABAC), rate limiting/throttling, input validation, error handling, logowanie, mTLS/TLS, secrets, dependency scanning, gateway/WAF config.
- Nie obejmuje: implementacji szczegółowej (w kodzie) ani IR/DR.

## Wejścia i wyjścia
- Wejścia: specyfikacja API, wymagania bezpieczeństwa/compliance, architektura (gateway/WAF/service mesh), profile ruchu, wymagania multi-tenant, lista integracji zewnętrznych.
- Wyjścia: dokumentacja decyzji architektonicznych (ADR), matryca authz, polityki rate limiting, wytyczne konfiguracji gateway/WAF, plan logowania/monitoringu, wymagania testów bezpieczeństwa.
...
1. Kontekst i zakres API (typ, odbiorcy, multi-tenant)
2. Authn/Authz (OAuth/OIDC, API keys, mTLS; scopes/claims/roles; tenant isolation)
3. Ochrona przed nadużyciami (rate limiting, throttling, bot/abuse, WAF/gateway reguły)
4. Walidacja danych i bezpieczeństwo transportu (schema validation, TLS/mTLS, HSTS)
5. Logowanie/monitoring/alerting (bez PII, trace_id, korelacja, detekcja anomalii)
6. Zarządzanie sekretami i zależnościami (KMS, rotacja, SBOM, skan zależności)
7. Wymagania testów bezpieczeństwa (SAST/DAST/API fuzzing, authz tests)
8. Zgodność i audyt (PCI/PII, retention logów, dowody)
