...
## Cel dokumentu
Opisać krok po kroku wykonanie backupu i odtworzenia danych/usług zgodnie z ustaloną strategią i RPO/RTO.

## Zakres i granice
Obejmuje: procedury backupu (full/inc/diff/snapshot), procedury restore (pełne/partial/PITR), walidację i raportowanie, eskalacje. Nie obejmuje: definicji strategii (jest w Strategy) ani projektowania retencji.

## Wejścia i wyjścia
- Wejścia: harmonogram i polityki, listy systemów/danych, narzędzia backupowe, klucze/szyfrowanie, RPO/RTO.
- Wyjścia: wykonane backupy/restore z logiem, wyniki walidacji, alert w razie niepowodzenia.
...
- Prerequisites (dostępy, klucze, storage)
- Procedura backupu (kroki, komendy, kolejność)
- Procedura odtwarzania (scenariusze: pełne, plik, punkt w czasie)
- Walidacja (checksumy, integralność, logi)
