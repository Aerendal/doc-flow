...
## Cel dokumentu
Opisuje, jak monitorować API: jakie metryki/logi/trace zbierać, progi alertów, ścieżkę triage i eskalacje, by utrzymać SLO (latencja, 4xx/5xx, availability).

## Zakres i granice
Obejmuje: źródła telemetryczne dla API, SLO/SLA, progi alertów, procedurę triage (klasyfikacja błędów, kontrakty), eskalacje, walidacje po incydencie. Nie obejmuje: naprawy kodu/usług (są w innych runbookach).

## Wejścia i wyjścia
- Wejścia: monitoring API (metryki, logi, trace), SLO/SLA, lista endpointów/krytyczności, testy kontraktowe, release/deploy timeline.
- Wyjścia: triage/eskalacja dla alertu, akcje tymczasowe (traffic shedding/retry), komunikaty statusowe, bilety do naprawy trwałej.
...
- Źródła telemetryczne (metryki, logi, trace) i dostęp
- SLO/SLA i progi alertów (latencja, error rate, availability, saturation)
- Triage alertów: klasyfikacja (5xx, 4xx spike, latency), kontrakty i backward compat
- Akcje natychmiastowe: throttle/rate-limit, circuit breaker, canary rollback
- Eskalacja i przekazanie do właścicieli usług
- Walidacja po incydencie i przywrócenie alertów do normy
